# 안티그래비티로 네이버 API 활용 보고서와 대시보드 만들기 
* 📺 유튜브 영상 ▶️ https://youtu.be/C8WxCXHSiko

[![YouTube](https://img.youtube.com/vi/C8WxCXHSiko/maxresdefault.jpg)](https://youtu.be/C8WxCXHSiko)
...
* 개발자 대상 API 중 일부 서비스가 네이버 클라우드 플랫폼의 'NAVER API Hub'로 이관됨에 따라, 아래 3종 API의 신규 신청을 중단하고 서비스를 단계적으로 종료
* https://console.ncloud.com/naver-api-hub/application/create

### NAVER API Hub 이관 안내

네이버 개발자센터에서 제공하던 일부 데이터랩 및 검색 API가 **NCP NAVER API Hub**로 이관됩니다.
...
현재 네이버 개발자센터에서 발급받은 API 키를 사용하고 있다면 아래 일정을 확인하여 NAVER API Hub 환경으로 이전해야 합니다.

### 이관 일정

| 날짜           | 변경 내용                        |
| ------------ | ---------------------------- |
| 2026년 6월 25일 | NCP NAVER API Hub 서비스 오픈     |
| 2026년 7월 31일 | 네이버 개발자센터에서 대상 API 신규 신청 중단  |
| 2027년 6월 30일 | 네이버 개발자센터에서 대상 API 서비스 전면 종료 |

### 2026년 7월 31일 이후 변경 사항

2026년 7월 31일부터 다음 작업은 네이버 개발자센터가 아닌 **NAVER API Hub**에서만 진행할 수 있습니다.

* API 신규 신청
* API 호출량 증량
* API 기능 추가 및 변경 요청

기존 네이버 개발자센터에서 신청 및 발급을 완료한 사용자는 발급일로부터 1년 동안 기존 환경을 계속 사용할 수 있습니다.

> 안내문에는 기존 환경 사용 대상이 `2026년 7월 25일 이전에 신청 및 발급을 완료한 고객`으로 표기되어 있습니다. 신규 신청 중단일인 2026년 7월 31일과 기준일이 다르므로 실제 적용 기준은 네이버 공식 공지를 확인해야 합니다.

### 2027년 6월 30일 이후 변경 사항

2027년 6월 30일 네이버 개발자센터의 대상 API 서비스가 전면 종료됩니다.

서비스 종료 이후에는 기존 네이버 개발자센터에서 발급받은 API 키를 통한 호출이 차단되며, NAVER API Hub에서 발급받은 인증 정보를 사용해야 합니다.

### 기존 사용자 권장 사항

기존 네이버 개발자센터 API를 사용하는 프로젝트는 서비스 중단을 방지하기 위해 다음 작업을 진행해야 합니다.

1. NCP 계정 및 NAVER API Hub 이용 환경을 준비합니다.
2. NAVER API Hub에서 사용할 API를 새로 신청합니다.
3. 새로운 인증 방식과 요청 URL을 확인합니다.
4. 프로젝트의 환경 변수와 API 호출 코드를 변경합니다.
5. 기존 API와 응답 구조 및 호출 결과가 동일한지 테스트합니다.
6. 2027년 6월 30일 이전에 운영 환경 전환을 완료합니다.

### 환경 변수 관리

API 인증 정보는 소스 코드에 직접 작성하지 않고 환경 변수로 관리하는 것을 권장합니다.
...
NAVER API Hub 이관 과정에서 환경 변수명이나 인증 방식이 변경될 수 있으므로, 실제 적용 시 NAVER API Hub 공식 가이드를 기준으로 설정해야 합니다.

### 참고

이 저장소의 예제 코드가 기존 네이버 개발자센터 API를 기준으로 작성된 경우, NAVER API Hub 전환 이후에는 일부 요청 URL, 인증 헤더 또는 환경 변수 설정을 수정해야 할 수 있습니다.
...
* [[애플리케이션 - NAVER Developers](https://developers.naver.com/apps/#/register)](https://www.ncloud.com/product/applicationService/naverApiHub)
